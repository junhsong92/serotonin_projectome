function [wbest,hbest,normbest] = nnmf_crossval(a,k,bin_mat,varargin)
...
% ---- Nested functions ----
    ...
    function cellout = loopBody(iter,S)
        ...
        [whtry{3},whtry{4},whtry{1}] = ...
            nnmf1(bin_mat,a,whtry{3},whtry{4},ismult,maxiter,tolfun,tolx,...
...
% -------------------
function [w,h,dnorm] = nnmf1(bin_mat, a,w0,h0,ismult,maxiter,tolfun,tolx,...
...
for j=1:maxiter
    ...
    dnorm = sqrt(sum(sum(d.^2))/sum(bin_mat(:)));
